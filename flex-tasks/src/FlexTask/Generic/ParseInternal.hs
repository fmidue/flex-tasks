...
instance Parse Bool where
  formParser = parseBool



instance Parse Double where
  formParser = parseDouble
...
instance Parse a => Parse (Maybe a) where
  formParser = do
    mValue <- optionMaybe $ try $ parseText emptyMarker
    case mValue of
...
parseInstanceSingleInputList parser = contents <* spaces
    where
