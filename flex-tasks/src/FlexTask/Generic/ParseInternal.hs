...
class Parse a where
  ...
  >>> parseTest (formParser @Bool) $ unpack $ fromJust $ formatAnswer [["yes"]]
  True

  >>> parseTest (formParser @[Double]) $ unpack $ fromJust $ formatAnswer [["2.0", "0.12e-12"]]
  [2.0,1.2e-13]

  >>> parseTest (formParser @(String,Integer)) $ unpack $ fromJust $ formatAnswer [["Good day"],["-100"]]
  ("Good day",-100)

  >>> parseTest (formParser @Int) $ unpack $ fromJust $ formatAnswer [["Test"]]
...
>>> parseTest (formParser @MyType) $ unpack $ fromJust $ formatAnswer [["1"]]
One
-}
...
>>> parseTest (formParser @[MyType]) $ unpack $ fromJust $ formatAnswer [["1","3"]]
[One,Three]
-}
...
>>> parseTest (formParser @(SingleInputList CustomInt)) $ unpack $ fromJust $ formatAnswer [["1, 2, 3"]]
...
>>> parseTest myParser $ unpack $ fromJust $ formatAnswer [["12345"]]
parse error at (line 1, column 1):
...

>>> parseTest (escaped myParser) $ unpack $ fromJust $ formatAnswer [["12345"]]
12345

>>> parseTest (escaped $ formParser @Int) $ unpack $ fromJust $ formatAnswer [["12345"]]
parse error at (line 1, column 3):
...
-}
...
>>> run English $ parseWithOrReport (formParser @Int) reportWithFieldNumber $ unpack $ fromJust $ formatAnswer [["1"]]
Just 1

>>> run German $ parseWithOrReport (formParser @Int) reportWithFieldNumber $ unpack $ fromJust $ formatAnswer [["Hello"]]
...
>>> run German $ parseInfallibly (formParser @SingleChoiceSelection) $ unpack $ fromJust $ formatAnswer [["1"]]
Just (SingleChoiceSelection {getAnswer = Just 1})

>>> run English $ parseInfallibly (formParser @(SingleInputList Double)) $ unpack $ fromJust $ formatAnswer [["Wrong input"]]
...
>>> let formatSubmission = unpack . fromJust . formatAnswer

>>> run English $ parseWithFallback (formParser @Int) (displayInputAnd errorHandling) (void $ formParser @Double) $ formatSubmission [["2"]]
Just 2

>>> run English $ parseWithFallback (formParser @Int) (displayInputAnd errorHandling) (void $ formParser @Double) $ formatSubmission [["2.5"]]
Error in """2.5""" : >>>>Fractions are not allowed<<<<
Nothing

>>> run English $ parseWithFallback (formParser @Int) (displayInputAnd errorHandling) (void $ formParser @Double) $ formatSubmission [["I'm a number"]]
